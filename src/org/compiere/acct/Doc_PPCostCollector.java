...
public class Doc_PPCostCollector extends Doc
{
	...
	protected Fact createComponentIssue(MAcctSchema as)
	{
		...
		final MProduct product = m_cc.getM_Product();
		MAccount debit = null ;
		String docBaseType = MDocType.get(getCtx(), m_cc.getPP_Order().getC_DocType_ID()).getDocBaseType();
		...
		if(m_cc.getScrappedQty().signum() != 0) {
			MAccount  credit2 = m_line.getAccount(ProductCost.ACCTTYPE_P_WorkInProcess, as);
			 ...
			 for (MCostDetail cd : getCostDetails())
				{
					...
					if (cost.scale() > as.getStdPrecision())
						cost = cost.setScale(as.getStdPrecision(), RoundingMode.HALF_UP);
					createLines(element, as, fact, product, debit, credit2, cost, m_cc.getScrappedQty());
				
		}
		}
		for (MCostDetail cd : getCostDetails())
		{
			if (MDocType.DOCBASETYPE_MaintenanceOrder.equals(docBaseType)) {
				 debit = m_line.getAccount(ProductCost.ACCTTYPE_P_MethodChangeVariance, as);
			}else {
