		...
		if (product != null && product.isStocked())
		{
			...
			if (docLine.getM_AttributeSetInstance_ID() == 0)
			{
				...
				for (int j = 0; j < mas.length; j++)
				{
					...
					for (int i = 0; i <  MStorageOnHand.getWarehouse(docLine.getCtx(), o_M_Warehouse_ID, product.getM_Product_ID(), ma.getM_AttributeSetInstance_ID(), null, true, true, docLine.getM_Locator_ID(), 
							docLine.get_TrxName(), false).length; i++) {
						MStorageOnHand  mStorageOnHand = MStorageOnHand.getWarehouse(docLine.getCtx(), o_M_Warehouse_ID, product.getM_Product_ID(), ma.getM_AttributeSetInstance_ID(), null, true, true, docLine.getM_Locator_ID(), docLine.get_TrxName(), false)[i];
					
					//for (MStorageOnHand  mStorageOnHand : MStorageOnHand.getWarehouse(docLine.getCtx(), o_M_Warehouse_ID, product.getM_Product_ID(), ma.getM_AttributeSetInstance_ID(), null, true, true, docLine.getM_Locator_ID(), docLine.get_TrxName(), false)) {
						
						//if(qtyparsial.compareTo(QtyMA.abs())< 0) {
//						if(mStorageOnHand.getQtyOnHand().compareTo(QtyMA.abs())>0) {
//							qtyparsial = QtyMA.abs();
//						}else {
...
//						}
						...
						if (!MStorageOnHand.add(docLine.getCtx(), M_Warehouse_ID,
								...
								Qty.negate(), mStorageOnHand.getDateMaterialPolicy(),
								docLine.get_TrxName()))
							{
							...
							//qtyparsial = qtyparsial.add(mStorageOnHand.getQtyOnHand());
						}
			...
			{
				System.out.println(incomingTrx);
				if (!incomingTrx) {	//	C- Customer Shipment - V- Vendor Return
					Qty = Qty.negate();
				}else {
					Qty = Qty.abs();
				...
				if (!MStorageOnHand.add(docLine.getCtx(), M_Warehouse_ID, 
					docLine.getM_Locator_ID(),
					docLine.getM_Product_ID(), 
					docLine.getM_AttributeSetInstance_ID(), Qty,datematerialpolicy ,docLine.get_TrxName()))
				{
					throw new AdempiereException(); //Cannot correct Inventory (MA)
				}
