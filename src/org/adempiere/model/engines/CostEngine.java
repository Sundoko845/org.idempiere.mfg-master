...
public class CostEngine
{
	...
	{
		return MCostElement.COSTINGMETHOD_Fifo;
	}
	...
	public BigDecimal getProductStandardCostPrice(MPPCostCollector cc, MProduct product, MAcctSchema as, MCostElement element)
	{
		...
		if(element.getCostingMethod().equals("F")) {
			d = new CostDimension(product,
					...
					DB.getSQLValue(null, "select m_attributesetinstance_id from PP_Order_Cost where pp_order_id = "+cc.getPP_Order_ID()+" and m_attributesetinstance_id > 0 and m_product_id = "+ product.getM_Product_ID()), //M_ASI_ID,
					1000002);	
		}else {
		 d = new CostDimension(product,
				as, as.getM_CostType_ID(),
...
//			}
			//
			for (MCostElement element : getCostElements(mtrx.getCtx(),costingMethod))
			{
				//
...
//					cd.setM_Transaction_ID(mtrx.get_ID());
	...
	private Collection<MCostElement> getCostElements(Properties ctx , String costingelement)
	{
		if(costingelement.equals("F")) {
			return MCostElement.getByCostingMethod(ctx, getCostingMethodFifo());
		}else {
	...
	public void createActivityControl(MPPCostCollector cc)
	{
		...
		{
			for (MCostElement element : getCostElements(cc.getCtx(),costingMethod))
			{
				...
				System.out.println("yoyoyoyo"+qty);
	...
	public void createUsageVariances(MPPCostCollector ccuv, BigDecimal costvarian, BigDecimal qtyjmltenagakerja)
	{
		...
		else
		{
			...
			I_AD_Workflow wf = MWorkflow.get(ccuv.getCtx(), ccuv.getPP_Order_Node().getAD_WF_Node().getAD_Workflow_ID());
		...
		{
			for (MCostElement element : getCostElements(ccuv.getCtx(),costingMethod))
			{
				...
				if(costvarian.compareTo(BigDecimal.ZERO)>0) {
				 amt = roundCost((price.multiply(qtyjmltenagakerja)).multiply(qty), as.getC_AcctSchema_ID());
				}else {
				 amt = roundCost(price.multiply(qty), as.getC_AcctSchema_ID());
				}
				//
				// Create / Update Cost Detail
				if(costvarian.compareTo(BigDecimal.ZERO)>0) {
	...
	public void createRateVariances(MPPCostCollector cc)
	{
		...
		for (MAcctSchema as : getAcctSchema(cc))
		{
			...
			for (MCostElement element : getCostElements(cc.getCtx(),costingMethod))
			{
				...
				//System.out.println(amtStd+" "+ amtActual);
				if (amtStd.compareTo(amtActual) == 0)
					continue;
				//
				if (ccrv == null)
				{
					//System.out.println("afafaf"+product.getName());
					ccrv = createVarianceCostCollector(cc, MPPCostCollector.COSTCOLLECTORTYPE_RateVariance);
				}
				//
				//if(amtStd.abs().subtract(amtActual.abs()).compareTo(BigDecimal.ZERO)<0) {
					
				createVarianceCostDetail(ccrv,
						(amtStd.abs().subtract(amtActual.abs())), qty.negate(),
						cd, null, as, element);
				//}
//				else {
//					createVarianceCostDetail(ccrv,
	...
	public void createMethodVariances(MPPCostCollector cc)
	{
		...
		{		
			for (MAcctSchema as : getAcctSchema(cc))
			{
				...
				//final String costingMethod = product.getCostingMethod(as);
				for (MCostElement element : getCostElements(cc.getCtx(),getCostingMethod()))
				{
