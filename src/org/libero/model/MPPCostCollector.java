...
public class MPPCostCollector extends X_PP_Cost_Collector implements DocAction , IDocumentLine
{
	...
	public static MPPCostCollector createCollector (MPPOrder order,
			...
			int durationSetup,
			BigDecimal duration,
			BigDecimal costvarian,
			BigDecimal qtyres,
			BigDecimal qtyjmltenagakerja
		)
	{
		qtyreserved = qtyres ; 
...
//	@Override
	public String prepareIt()
	{
		...
		if(isActivityControl())
		{
			...
			System.out.println("ffff"+(getDurationReal().setScale(0, RoundingMode.HALF_EVEN)));
			activity.setDurationReal(activity.getDurationReal()+ Integer.parseInt((getDurationReal().setScale(0, RoundingMode.HALF_UP)).toString()));
			activity.setSetupTimeReal(activity.getSetupTimeReal()+ Integer.parseInt((getSetupTimeReal().setScale(0, RoundingMode.HALF_UP)).toString()));
			activity.saveEx(get_TrxName());
...
//	@Override
	...
	public String completeIt()
	{
		...
		if(isIssue() || isReceipt())
		{
			...
			String docBaseType = MDocType.get(getCtx(), getPP_Order().getC_DocType_ID()).getDocBaseType();
		...
		else if (isCostCollectorType(COSTCOLLECTORTYPE_UsegeVariance) && getPP_Order_BOMLine_ID() > 0)
		{
			...
			obomline.saveEx(get_TrxName());
			log.fine("OrderLine -> Reserved="+obomline.getQtyReserved()+", Delivered="+obomline.getQtyDelivered());
			CostEngineFactory.getCostEngine(getAD_Client_ID()).createUsageVariances(this,Env.ZERO,Env.ZERO);
		}
		//
		...
		else if (isCostCollectorType(COSTCOLLECTORTYPE_UsegeVariance) && getPP_Order_Node_ID() > 0)
		{
			...
			activity.saveEx(get_TrxName());
			CostEngineFactory.getCostEngine(getAD_Client_ID()).createUsageVariances(this,costva, jmltenagakerja);
		}
		else
		{
			; // nothing
...
//		}
		...
		setDescription(String.valueOf(DB.getSQLValue(null, "select count(*) from pp_cost_collector where pp_order_id = "+getPP_Order_ID()+" and costcollectortype = '100' and docstatus = 'CO'")));
...
//	@Override
	public boolean voidIt()
	{
		...
		MPPCostCollector collector = new MPPCostCollector(getCtx(), getPP_Cost_Collector_ID(), get_TrxName());
		if(collector.getCostCollectorType().equals(COSTCOLLECTORTYPE_MaterialReceipt)) {
			
		StorageEngine.createTransaction(
				this,
				MTransaction.MOVEMENTTYPE_WorkOrder_ , 
				new Timestamp(new Date().getTime()) , 
				collector.getMovementQty() , 
				true,											// IsReversal=false
				collector.getM_Warehouse_ID(), 
				collector.getM_AttributeSetInstance_ID(),	// Reservation ASI
				collector.getM_Warehouse_ID(),				// Reservation Warehouse
				false											// IsSOTrx=false
				);
		}else if(collector.getCostCollectorType().equals(COSTCOLLECTORTYPE_ComponentIssue)) {
			StorageEngine.createTransaction(
					this,
					MTransaction.MOVEMENTTYPE_WorkOrderPlus , 
					new Timestamp(new Date().getTime()) , 
					collector.getMovementQty().negate() , 
					true,											// IsReversal=false
					collector.getM_Warehouse_ID(), 
					collector.getM_AttributeSetInstance_ID(),	// Reservation ASI
					collector.getM_Warehouse_ID(),				// Reservation Warehouse
		...
		if(collector.getCostCollectorType().equals(COSTCOLLECTORTYPE_MaterialReceipt)) {
			...
			if(order.getDocStatus().equals(DOCSTATUS_Closed)) {
				order.setDocStatus(DOCSTATUS_Completed);
				order.setDocAction(DOCACTION_Close);
			}
			//
			// Update PP Order Dates
//			order.setDateDelivered(getMovementDate()); //	overwrite=last
...
//				log.fine("OrderLine - Reserved=" + obomline.getQtyReserved() + ", Delivered=" + obomline.getQtyDelivered());				
			obomline.saveEx(get_TrxName());
		//	log.fine("OrderLine -> Reserved="+obomline.getQtyReserved()+", Delivered="+obomline.getQtyDelivered());
		}else if(collector.getCostCollectorType().equals(COSTCOLLECTORTYPE_ActivityControl)) {
			...
			orderNode.set_CustomColumn("qtyreserved", orderNode.getQtyRequired().subtract(orderNode.getQtyDelivered().subtract(collector.getMovementQty())));
		...
		//	orderNode.set_CustomColumn("qtyreserved", orderNode.getQtyRequired().subtract(collector.getMovementQty().add (new BigDecimal(orderNode.get_Value("qtyreserved").toString()))));
			...
			if(orderNode.getDocStatus().equals(DOCSTATUS_Closed) || orderNode.getDocStatus().equals(DOCSTATUS_InProgress)) {
				if(qtydeliver.compareTo(BigDecimal.ZERO)==0) {
					orderNode.setDocStatus(DOCSTATUS_Drafted);
					orderNode.setDocAction(DOCACTION_Complete);
				}else {
					orderNode.setDocStatus(DOCSTATUS_InProgress);
					orderNode.setDocAction(DOCACTION_Complete);
				}
			}
			orderNode.saveEx();
		...
		DB.executeUpdate("delete from Fact_Acct where record_id = " + collector.getPP_Cost_Collector_ID(), get_TrxName());
		if(DB.getSQLValueString(get_TrxName(), "select coalesce(costingmethod,'') from M_Product_Category_Acct where m_product_category_id = "+ collector.getM_Product().getM_Product_Category_ID()).equals("F")) {
			 DB.executeUpdate("update m_cost set currentqty = (select sum(qtyonhand) from m_storage where m_product_id = "+collector.getM_Product_ID()+" and M_AttributeSetInstance_ID = "+ collector.getM_AttributeSetInstance_ID()+") where m_product_id = "+collector.getM_Product_ID()+" and M_AttributeSetInstance_ID = "+collector.getM_AttributeSetInstance_ID(),get_TrxName());	
			 DB.executeUpdate("update M_CostQueue set currentqty = (select sum(qtyonhand) from m_storage where m_product_id = "+collector.getM_Product_ID()+" and M_AttributeSetInstance_ID = "+ collector.getM_AttributeSetInstance_ID()+") where m_product_id = "+collector.getM_Product_ID()+" and M_AttributeSetInstance_ID = "+collector.getM_AttributeSetInstance_ID(),get_TrxName());	
		}else {
			 DB.executeUpdate("update m_cost set currentqty = (select sum(qtyonhand) from m_storage where m_product_id = "+collector.getM_Product_ID()+") where m_product_id = "+collector.getM_Product_ID()+" and M_CostElement_ID=1000000 ",get_TrxName());
		}
	    return true;
	}	//	voidIt
