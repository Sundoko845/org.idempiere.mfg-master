...
public class MPPOrderBOMLine extends X_PP_Order_BOMLine
{
		...
		if(m_isExplodePhantom && m_qtyRequiredPhantom != null)
		{
			...
			if (bom != null)
			{
				for(MPPProductBOMLine PP_Product_BOMline : bom.getLines())
				{
					...
					PP_Order_BOMLine.setQtyPlusScrap(m_qtyRequiredPhantom, BigDecimal.ONE,BigDecimal.ONE,null,BigDecimal.ONE);
	 ...
	 * @param QtyOrdered
	 */
	public void setQtyPlusScrap(BigDecimal QtyOrdered, BigDecimal qtybatchsize, BigDecimal qtybatchparent, MProduct product, BigDecimal qtyorderparent )
	{
		BigDecimal qty = BigDecimal.ZERO;
		BigDecimal multiplier = getQtyMultiplier();	
		int conv =DB.getSQLValue(get_TrxName(), "select count(*) from C_UOM_Conversion where m_product_id = "+product.getM_Product_ID());
		BigDecimal qtyconv = BigDecimal.ONE;
		System.out.println("adfafaf"+product.getM_Product_ID());
		System.out.println("sdsff"+conv);
		if(conv>0) {
			int idconv =DB.getSQLValue(get_TrxName(), "select  C_UOM_Conversion_ID from C_UOM_Conversion where m_product_id = "+product.getM_Product_ID());
			MUOMConversion conversion = new MUOMConversion(Env.getCtx(),idconv, get_TrxName());
			qtyconv = conversion.getDivideRate();
			System.out.println("yyyyy"+qtyconv+" "+getQtyBOM());
			if(qtyconv.compareTo(getQtyBOM())==0) {
				qty = QtyOrdered.round(new MathContext(6, RoundingMode.FLOOR));			}	
			else {
				qty = qtyorderparent.multiply(multiplier).setScale(6, RoundingMode.FLOOR);
			}
		
		}else {
		qty = QtyOrdered.multiply(multiplier).setScale(8, RoundingMode.FLOOR);
		}
