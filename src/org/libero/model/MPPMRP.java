...
public class MPPMRP extends X_PP_MRP implements DocAction
{
    ...
    public static int createMOFromForecast(MForecastLine ol, BigDecimal qty) {
        ...
        MPPMRP mrp = new MPPMRP(ol.getCtx(), DB.getSQLValue(ol.get_TrxName(),"select pp_mrp_id from pp_mrp where m_forecastline_id = " + ol.getM_ForecastLine_ID()), ol.get_TrxName());
		System.out.println(mrp.getPP_MRP_ID());
...
//red1 raised the getPlantForWareHouse as later stops setC_OrderLine in PP_Order (see http://red1.org/adempiere/viewtopic.php?f=45&t=1775&p=8471#p8471)
		int plant_id = ol.get_ValueAsInt("S_Resource_ID");
		...
		if(bom == null || workflow == null)
		{
			...
			pp = MPPProductPlanning.find(ol.getCtx(), ol.getAD_Org_ID(), ol.getM_Warehouse_ID() , plant_id , ol.getM_Warehouse_ID(), ol.get_TrxName()); 	
			if(pp == null)
		...
		//MPPOrder order = MPPOrder.forM_ForecastLine_ID(ol.getCtx(), ol.get_ID(), ol.get_TrxName());
...
//				+ ol.getParent().getName();
				...
				pp.setS_Resource_ID(plant_id);
				 ...
				 MPPMRP.createMOForecast(mrp,pp, ol.getM_ForecastLine_ID(),0, 
										qty, ol.getDatePromised(), ol.getDatePromised(),ol.getDatePromised(),ol.getDatePromised(), "Generated From Forecast"+ol.getM_Forecast_ID(),ol.getM_ForecastLine_ID());
				
//				description = "";
//				if(ol.get_ValueAsString("Description") != null)
//					description = ol.get_ValueAsString("Description");
//				
...
////							+ order.getDocumentNo();
//				
//				ol.set_CustomColumn("Description", description);
//				ol.saveEx();
			//}
		}
		return 0;
...
//red1 raised the getPlantForWareHouse as later stops setC_OrderLine in PP_Order (see http://red1.org/adempiere/viewtopic.php?f=45&t=1775&p=8471#p8471)
									...
									Timestamp dateOrdered,Timestamp datePromised,String description)
	{
	   ...
	   public static void costPPorder (MPPOrder pporder){
			MAcctSchema as = new MAcctSchema(Env.getCtx(),1000000 , pporder.get_TrxName());	
			MProduct product = new MProduct(Env.getCtx(), pporder.getM_Product_ID(), pporder.get_TrxName());
			 String costingMethod = product.getCostingMethod(as);
			MCostElement element = MCostElement.getByCostingMethod(Env.getCtx(), costingMethod).get(0);
		...
		public static void costQueuePPorder (MPPOrder mppOrder){
			MAcctSchema acctSchema = new MAcctSchema(Env.getCtx(),1000000 , mppOrder.get_TrxName());
			MProduct product = new MProduct(Env.getCtx(), mppOrder.getM_Product_ID(),mppOrder.get_TrxName());
			 String costingMethod = product.getCostingMethod(acctSchema);
			MCostElement element = MCostElement.getByCostingMethod(Env.getCtx(), costingMethod).get(0);
					if(mppOrder.getM_AttributeSetInstance_ID()>0) {					
						     
							  MCostQueue costQueue = new MCostQueue(product, mppOrder.getM_AttributeSetInstance_ID(), acctSchema, 0, element.getM_CostElement_ID(), mppOrder.get_TrxName());
	...
	public static MPPOrder createMOForecast(MPPMRP mrp,MPPProductPlanning pp,int C_OrderLine_ID,int M_AttributeSetInstance_ID , BigDecimal qty, 
			Timestamp dateOrdered,Timestamp datePromised,Timestamp startsch,Timestamp finishsch,String description,int forecastlinedetail)
{
...
int idforecast =  DB.getSQLValue(null, "select coalesce(pp_order_id,0) from pp_order where m_forecastlinedetail_id = ?", forecastlinedetail);
if(idforecast > 0) {
	MPPOrder order = new MPPOrder(pp.getCtx(), idforecast , pp.get_TrxName());
	order.setAD_Org_ID(pp.getAD_Org_ID());
	order.setDescription(description);
	order.set_CustomColumn("M_ForecastLine_ID", C_OrderLine_ID);
	order.set_CustomColumn("m_forecastlinedetail_ID", forecastlinedetail);
	order.setS_Resource_ID(pp.getS_Resource_ID());
	order.setM_Warehouse_ID(pp.getM_Warehouse_ID());
	order.setM_Product_ID(pp.getM_Product_ID());
	order.setM_AttributeSetInstance_ID(M_AttributeSetInstance_ID);
	...
	order.setDateOrdered(dateOrdered);                       
	order.setDatePromised(datePromised);
	order.setDateStartSchedule(startsch);
	order.setDateFinishSchedule(finishsch);
	//red1 passing MO to WFNode resource assign within to keep track
//	int duration = routingService.calculateDuration(mrp, wf,MResource.get(pp.getCtx(), pp.getS_Resource_ID()), qty, datePromised).intValueExact();		//Ferry routingService.calculateDuration(mrp, wf,MResource.get(pp.getCtx(), pp.getS_Resource_ID()),qty).intValueExact(); 
//	//
//	
...
//	order.setDateFinishSchedule(TimeUtil.addMinutess(startTime, duration)); //Ferry duration in minutes order.setDateFinishSchedule(TimeUtil.addDays(startTime, duration));
	...
	order.setDateOrdered(dateOrdered);                       
	order.setDatePromised(datePromised);
	order.setDateStartSchedule(datePromised);
	order.setDateStartSchedule(startsch);
	order.setDateFinishSchedule(finishsch);
...
return null;
	...
	//	mrp.set_CustomColumn("pp_orderref_id", ol.ge);
	...
	public static void PP_Order(MPPOrder o)
	{
		...
		//mrpSupply.set_CustomColumn("PP_OrderRef_ID", o.getPP_Order_ID());
   ...
   public static BigDecimal getQtyOrdered(Properties ctx, int M_Warehouse_ID ,int M_Product_ID,String trxName)
   {
	...
	public static int getDurationMinutes(MPPMRP mrp, BigDecimal qty, I_PP_Product_Planning pp, Timestamp DemandDateStartSchedule)
	{
		...
		MProduct product = MProduct.get(ctx, pp.getM_Product_ID());
		System.out.println("Duration"+product.getM_Product_ID());
		BigDecimal leadtime = pp.getDeliveryTime_Promised().multiply(DayMinutes); //DeliveryTime unit is days
		System.out.println("Durationleadtime"+leadtime+" "+pp.getDeliveryTime_Promised());
		if (leadtime.signum() != 0 || product.isPurchased())
		{
			;
		}
