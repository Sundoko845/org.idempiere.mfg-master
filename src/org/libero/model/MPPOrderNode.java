...
//		set_ValueOfColumn("IsEmployee", wfNode.get_Value("IsEmployee"));
	...
	public void closeIt()
	{
		...
		{	
			...
			setDurationRequired(getDurationReal() +  Integer.parseInt(new BigDecimal(get_ValueAsString("qtyreserved")).setScale(0,RoundingMode.HALF_UP).toString())  * getDuration());
			setQtyRequired(getQtyDelivered().add(new BigDecimal(get_ValueAsString("qtyreserved"))));			
		}	
	}
	...
	protected boolean beforeSave(boolean newRecord) {
		if(!newRecord) {
			int product = DB.getSQLValue(null, "select m_product_id from m_product where name = '"+getS_Resource().getName()+"'");
			System.out.println("id"+product );
            BigDecimal cost = DB.getSQLValueBD(null, "select sum(currentcostprice) from m_cost where M_CostElement_ID != 1000000  and m_product_id = " + product); 
			System.out.println("cost" + cost);
            setCost(new BigDecimal((getWaitingTime()+getDuration()+getSetupTime())*Integer.parseInt(get_ValueAsString("jmltenagakerja")) ).multiply(cost) );
			MPPOrderWorkflow mppOrderWorkflow = new MPPOrderWorkflow(getCtx(), getPP_Order_Workflow_ID(), get_TrxName());
			mppOrderWorkflow.setCost(DB.getSQLValueBD(get_TrxName(), "select coalesce(sum(cost),0) from PP_Order_Node where PP_Order_Node_ID !=  " + getPP_Order_Node_ID() +" and pp_order_id = "+getPP_Order_ID()).add(new BigDecimal((getWaitingTime()+getDuration())*get_ValueAsInt("jmltenagakerja")).multiply(cost) ));
	...
	public boolean isProcessed()
	{
		...
		System.out.println("Sfsfsfs"+status+getPP_Order_Node_ID());
