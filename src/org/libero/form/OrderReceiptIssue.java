	...
	public void createIssue(MPPOrder order, IMiniTable issue) {
		...
		for (int i = 0; i < m_issue.length; i++) {
			...
			if (product != null && product.get_ID() != 0 && product.isStocked()) {
				...
				if(DB.getSQLValueString(order.get_TrxName(), "select coalesce(mpc.costingmethod,'') from M_Product_Category_Acct mpc where mpc.m_product_category_id = "+ 
				product.getM_Product_Category_ID()).equals("F") && value == null ) {
				if ( isSelected  ) {
				...
				if(!DB.getSQLValueString(order.get_TrxName(), "select coalesce(mpc.costingmethod,'') from M_Product_Category_Acct mpc where mpc.m_product_category_id = "+ 
						product.getM_Product_Category_ID()).equals("F")){
...
//						false);
	...
	public void executeQuery(IMiniTable issue) {
		...
		try {
			...
			while (rs.next()) {
				// extend table
				//issue.setColumnReadOnly(row, true);
				...
				if (componentType
					...
					if (isQtyPercentage) {
						...
						if (isBackflush()) { // Is Backflush - Calculate
							m_PP_order = getPP_Order();				// Component from Qty To Deliver
							int conv =DB.getSQLValue(null, "select count(*) from C_UOM_Conversion where m_product_id = "+m_PP_order.getM_Product_ID());
							if(conv > 0) {
								int idconv =DB.getSQLValue(null, "select  C_UOM_Conversion_ID from C_UOM_Conversion where m_product_id = "+m_PP_order.getM_Product_ID());
								MUOMConversion conversion = new MUOMConversion(Env.getCtx(),idconv, null);
								if(qtyBom.compareTo( conversion.getDivideRate())==0) {
									componentToDeliverQty = toDeliverQty;
											 // TODO: set Number scale
									if (componentToDeliverQty.signum() != 0) {
										componentQtyReq = toDeliverQty;
										componentQtyToDel = componentToDeliverQty;
	...
	private int lotes(int row, IDColumn id, int Warehouse_ID, int M_Product_ID,
		...
		System.out.println("Productco"+M_Product_ID);
		...
		try {
			...
			while (rs.next()) {
				...
				// ASI
				KeyNamePair keynamepairasi = new KeyNamePair(rs.getInt(3),
						rs.getString(5));
				issue.setValueAt(keynamepairasi, row, 5);
				// Locator
				...
				issue.setValueAt(qtyRequired, row, 6); // QtyRequired
				//issue.setValueAt(qtyToDelivery, row, 8); // QtyToDelivery
				...
				if (qtyRequiredActual.compareTo(qtyOnHand) < 0) {
					...
					issue.setValueAt(qtyOnHand, row, 8);
				}
