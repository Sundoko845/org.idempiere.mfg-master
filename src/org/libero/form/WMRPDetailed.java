...
public class WMRPDetailed extends MRPDetailed implements IFormController, EventListener, ListDataListener, WTableModelListener {
	...
	private Label lorder = new Label(Msg.translate(getCtx(), MPPMRP.COLUMNNAME_C_Order_ID)); 
	private WSearchEditor forder_id;
	private Label lMO_ID = new Label(Msg.translate(getCtx(), MPPMRP.COLUMNNAME_PP_Order_ID));
	...
	private void statInit() throws Exception
	{
		...
		forder_id = new WSearchEditor(MPPMRP.COLUMNNAME_C_Order_ID, true, false, true,
				MLookupFactory.get (getCtx(), p_WindowNo, 0, MColumn.getColumn_ID(MOrder.Table_Name,MPPMRP.COLUMNNAME_C_Order_ID), DisplayType.Search)) 
		{
			private final long serialVersionUID = 1L;
			public void setValue(Object arg0) {  				
				super.setValue(arg0);
		...
		fMO_ID = new WSearchEditor(MPPMRP.COLUMNNAME_PP_Order_ID, true, false, true,
				MLookupFactory.get (getCtx(), p_WindowNo, 0, MColumn.getColumn_ID(MPPOrder.Table_Name,MPPMRP.COLUMNNAME_PP_Order_ID), DisplayType.Search)) 
		{
			private final long serialVersionUID = 1L;
			public void setValue(Object arg0) {  				
				super.setValue(arg0);
		...
		//  9th Row
		row = rows.newRow();
		row.appendChild(lMO_ID.rightAlign());
		row.appendChild(fMO_ID.getComponent());
		row.appendChild(new Space());
	...
	private String getSQLWhere()
	{
		 ...
		 if(forder_id.getValue() != null) {
			 sql.append(" AND "+getTableName()+".C_Order_ID=?");
			 //sql.append(" OR (("+getTableName()+".OrderType IN ('SOO','MOP','POO','POR','STK','DOO')) OR ("+getTableName()+".OrderType='FCT' AND "+getTableName()+".DatePromised >= SYSDATE))");
		 }
		 
		 if(fMO_ID.getValue() != null) {
