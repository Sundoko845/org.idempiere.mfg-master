...
public class MRP extends SvrProcess
{
	...
	protected void prepare()
	{
		...
		for (int i = 0; i < para.length; i++)
		{
			...
			p_M_Order_ID = para[i].getParameterAsInt();
		    }
			else
		...
		{	
			...
			for (MOrg org : orgList)
			{
				...
				for(MWarehouse w : ws)
				{
					...
					runMRP(getAD_Client_ID(), org.getAD_Org_ID(), plant.getS_Resource_ID(), w.getM_Warehouse_ID(),getC_Order_ID(),getPP_Order_ID());
		...
		try
		{
			...
			for (int level = 0 ; level <= lowlevel ; level++)
			{
				...
				String sql = "SELECT mrp.M_Product_ID, mrp.LowLevel, mrp.Qty, mrp.DatePromised"
							...
							+(C_Order_ID > 0 ? " AND mrp.C_Order_ID = "+C_Order_ID : "")
							+(PP_Order_ID > 0 ? " AND mrp.PP_Order_ID = "+PP_Order_ID : "")
...
//							+(p_M_Product_ID > 0 ? " AND mrp.M_Product_ID="+p_M_Product_ID : "")
				...
				//pstmt.setInt(7, C_Order_ID);
				//pstmt.setInt(7, PP_Order_ID);
				...
				//	 if demand is forecast and promised date less than or equal to today, ignore this QtyGrossReq
					...
					if (product == null || product.get_ID() != M_Product_ID)
					{
						...
						if (QtyGrossReqs.signum() != 0)
						{
									...
									&& POQDateStartSchedule.compareTo(Planning_Horizon) < 0) 
							{
								...
								calculatePlan(AD_Client_ID, AD_Org_ID,M_Warehouse_ID ,BeforePP_MRP_ID , product ,BeforeDateStartSchedule,C_Order_ID);
					...
					{
						calculatePlan(AD_Client_ID,AD_Org_ID,M_Warehouse_ID,PP_MRP_ID,product ,DatePromisedFrom,C_Order_ID);						
					...
					{       
						...
						calculatePlan(AD_Client_ID, AD_Org_ID,M_Warehouse_ID,PP_MRP_ID,product,BeforeDateStartSchedule,C_Order_ID); 		
						continue;
					}                                                                        
				} // end while
	...
	private void calculatePlan(int AD_Client_ID, int AD_Org_ID, int M_Warehouse_ID, int PP_MRP_ID,
								MProduct product, Timestamp DemandDateStartSchedule,int order_id) throws SQLException
	{
		...
		if (QtyPlanned.signum() > 0)    
		{
			...
			for (int ofq = 1 ; ofq <= loops ; ofq ++ )
			{
				...
				try
				{
					createSupply(AD_Org_ID, PP_MRP_ID, product, QtyPlanned, DemandDateStartSchedule,order_id);
				}
				catch (Exception e)
				{
	 ...
	 * @throws SQLException 
	 */
	protected void createSupply(int AD_Org_ID, int PP_MRP_ID, MProduct product, BigDecimal QtyPlanned ,Timestamp DemandDateStartSchedule, int orderid)
	throws AdempiereException, SQLException
	{
		...
		{
 			createRequisition(AD_Org_ID, PP_MRP_ID, product, QtyPlanned ,DemandDateStartSchedule,orderid);
		}
		// Manufacturing Order
	...
	protected void createRequisition(int AD_Org_ID, int PP_MRP_ID, MProduct product, BigDecimal QtyPlanned, Timestamp DemandDateStartSchedule, int order_id)
	throws AdempiereException, SQLException
	{
