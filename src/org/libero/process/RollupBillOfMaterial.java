...
public class RollupBillOfMaterial extends SvrProcess
{
	...
	protected void rollup(MProduct product, MPPProductBOM bom)
	{
		for (MCostElement element : getCostElements())
		{
			...
			{        
				...
				System.out.println(element.getName() + " Cost Low Level:" + price);
	...
	private BigDecimal getCurrentCostPriceLL(MPPProductBOM bom, MCostElement element)
	{
		...
		System.out.println("afagagagagag2"+ costPriceLL );// for each BOM line
		return costPriceLL;     
	}
	...
	private Collection<MCost> getCosts(MProduct product, int M_CostElement_ID)
	{
		...
		//	 d = new CostDimension(product, as, p_M_CostType_ID, p_AD_Org_ID, 0, M_CostElement_ID);
		//}else {
		//	 d = new CostDimension(product, as, p_M_CostType_ID,p_AD_Org_ID, DB.getSQLValue(null, "select m_attributesetinstance_id from m_cost where m_product_id = ?", product.getM_Product_ID()), M_CostElement_ID);
		//}
		CostDimension d = new CostDimension(product, as, p_M_CostType_ID, p_AD_Org_ID, DB.getSQLValue(null, "select m_attributesetinstance_id from m_cost where m_product_id = ?", product.getM_Product_ID()), M_CostElement_ID);
