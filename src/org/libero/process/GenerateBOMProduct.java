...
public class GenerateBOMProduct extends SvrProcess {
	...
	private BigDecimal Qtyonhandproject = Env.ZERO;
	...
	protected String doIt() throws Exception {
		...
		this.Planning_Horizon = TimeUtil.addDays(getToday(), DB.getSQLValue(null, "select planninghorizon from S_Resource "
				+ " where S_Resource_ID = " + mppOrder.getS_Resource_ID()));
		List<MPPMRP> listmrp = new Query(getCtx(), MPPMRP.Table_Name, "pp_orderref_id::integer = "+mppOrder.getPP_Order_ID()+" and coalesce(pp_order_id,0) != "+mppOrder.getPP_Order_ID(), get_TrxName()).list();
		...
		for(MPPMRP mppmrp  : listmrp) {
			...
			DB.executeUpdate("delete from pp_mrp where pp_mrp_id = "+mppmrp.getPP_MRP_ID(), get_TrxName());
		}
		deletemrp();
//		try {
...
//		e1.printStackTrace();
//	}
		MProduct product = MProduct.get(getCtx(), mppOrder.getM_Product_ID());
					...
					for (int i = 0; i < list.size(); i++) {
						...
						if(productmasterbom == list.get(i).getProductparent()) {
							...
							if(DB.getSQLValueString(get_TrxName(), "select isbom from m_product where m_product_id = " + productid).equals("Y"))
							{
								...
								//Qty = (list.get(i).getQtyrequired().multiply(qtybom)).add(DB.getSQLValueBD(get_TrxName(), "select coalesce(sum(qtyonhand),0) from m_storage where m_product_id = "+productid+" having sum(qtyonhand) > 0"));
								Qty = (list.get(i).getQtyrequired().multiply(qtybom));
								BigDecimal QtyDemand = DB.getSQLValueBD(null, "select coalesce(sum(qty),0) from pp_mrp where DocStatus = 'DR' and ad_org_id = "+AD_Org_ID+" and m_warehouse_id = "+M_Warehouse_ID+" and m_product_id = " +productid+" and s_resource_id = "+S_Resource_ID+" and TypeMRP = '"+MPPMRP.TYPEMRP_Demand+"' and pp_orderref_id::integer != " + PP_Order_ID);
							    BigDecimal QtySupply = DB.getSQLValueBD(null, "select coalesce(sum(qty),0) from pp_mrp pp inner join  m_product mp on mp.m_product_id = pp.m_product_id where  DocStatus = 'DR' and pp.ad_org_id = "+AD_Org_ID+" and pp.m_warehouse_id = "+M_Warehouse_ID+" and pp.m_product_id = " +productid+" and pp.s_resource_id = "+S_Resource_ID+" and pp.TypeMRP = '"+MPPMRP.TYPEMRP_Supply+"' and pp_orderref_id::integer != " + PP_Order_ID);
							    if(QtyDemand.compareTo(BigDecimal.ZERO) > 0 && QtySupply.compareTo(BigDecimal.ZERO) > 0 ) {
							    Qtyonhandproject = QtyDemand.subtract(QtySupply);	
							    }
//							    else if(QtyDemand.compareTo(BigDecimal.ZERO) > 0 ) {
//							    	Qtyonhandproject = QtyDemand;
//							    }
								...
								BigDecimal QtyDemand = DB.getSQLValueBD(null, "select  coalesce(sum(qty),0) from pp_mrp where DocStatus = 'DR' and ad_org_id = "+AD_Org_ID+" and m_warehouse_id = "+M_Warehouse_ID+" and m_product_id = " +productid+" and s_resource_id = "+S_Resource_ID+" and TypeMRP = '"+MPPMRP.TYPEMRP_Demand+"' and pp_orderref_id::integer != " + PP_Order_ID);
							    BigDecimal QtySupply = DB.getSQLValueBD(null, "select coalesce(sum(qty),0) from pp_mrp pp inner join  m_product mp on mp.m_product_id = pp.m_product_id where  DocStatus = 'DR' and pp.ad_org_id = "+AD_Org_ID+" and pp.m_warehouse_id = "+M_Warehouse_ID+" and pp.m_product_id = " +productid+" and pp.s_resource_id = "+S_Resource_ID+" and pp.TypeMRP = '"+MPPMRP.TYPEMRP_Supply+"' and pp_orderref_id::integer != " + PP_Order_ID);
//							    System.out.println("QtyDemand"+ QtyDemand+ "- "+ list.get(i).getProductparent());
//							    System.out.println("QtyDemand"+ QtySupply+ "- "+ list.get(i).getProductparent());
							    //Qtyonhandproject = QtyDemand.subtract(QtySupply);
							    if(QtyDemand.compareTo(BigDecimal.ZERO) > 0 && QtySupply.compareTo(BigDecimal.ZERO) > 0 ) {
								    Qtyonhandproject = QtyDemand.subtract(QtySupply);	
								    }
...
//					if()
//					Qtyonhandproject = DB.getSQLValueBD(null, "select sum(qty) from pp_mrp where DocStatus = 'DR' and ad_org_id = "+AD_Org_ID+" and m_warehouse_id = "+M_Warehouse_ID+" and m_product_id = " +productid+" and s_resource_id = "+S_Resource_ID+" and TypeMRP = '"+MPPMRP.TYPEMRP_Demand+"'")
...
//				    System.out.println("QTY"+Qty);
					...
					final int M_Product_ID = productid ; 
						...
						setProduct(AD_Client_ID,AD_Org_ID ,S_Resource_ID , M_Warehouse_ID,  product,Qtyonhandproject);
...
//		}
	 ...
	 * @throws SQLException 
	 */
	private void setProduct(int AD_Client_ID , int AD_Org_ID, int S_Resource_ID , int M_Warehouse_ID, MProduct product, BigDecimal qtyonhandproject) throws SQLException
	{
		DatePromisedTo = null;
        ...
        System.out.println("YOYO"+qtyonhandproject);
		QtyProjectOnHand = getQtyOnHand(m_product_planning).subtract(qtyonhandproject);
		System.out.println("onhand"+QtyProjectOnHand);
	...
	throws AdempiereException, SQLException
	{
		...
		MPPMRP mppmrp = new MPPMRP(getCtx(), PP_MRP_ID, get_TrxName());
		...
		// Set Correct Dates for Plan
				...
				System.out.println("QTYPLN"+QtyPlanned+" "+ product.getName());
